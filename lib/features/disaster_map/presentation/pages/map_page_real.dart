...
class _MapPageRealState extends State<MapPageReal> {
  ...
  Future<void> _openGoogleMaps(String latStr, String longStr) async {
    // 1. Coba Format URL Universal (Bisa Browser / App)
    final Uri googleUrl = Uri.parse(
      'https://www.google.com/maps/dir/?api=1&destination=$latStr,$longStr&travelmode=driving',
    );

    try {
      // Mode externalApplication memaksa buka di App Maps / Chrome
  ...
  Map<String, dynamic> _getMarkerStyle(String category) {
    ...
    // 2. Kategori Lainnya
    if (cat.contains('banjir'))
      return {'color': Colors.blue, 'icon': Remix.rainy_fill};
    if (cat.contains('longsor'))
      return {'color': Colors.brown, 'icon': Remix.alert_fill};
    if (cat.contains('kebakaran'))
      return {'color': Colors.red, 'icon': Remix.fire_fill};
    if (cat.contains('jalan'))
      return {'color': Colors.grey[700]!, 'icon': Remix.road_map_fill};
    if (cat.contains('pohon'))
      return {'color': Colors.green, 'icon': Remix.tree_fill};
    if (cat.contains('sampah'))
      return {'color': Colors.orange, 'icon': Remix.delete_bin_2_fill};
  ...
  List<Marker> _buildMarkers() {
    return _displayedReports.map((report) {
      ...
      bool isSOS =
          report.category.toLowerCase().contains('darurat') ||
          report.category.toLowerCase().contains('sos');
        ...
        child: GestureDetector(
          ...
          child: Container(
            decoration: BoxDecoration(
              color: isSOS
                  ? Colors.red[50]
                  : Colors.white, // Background merah muda jika SOS
          ...
          child: Column(
            ...
            children: [
                ...
                children: [
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: isSOS
                          ? Colors.redAccent
                          : style['color'].withOpacity(0.1), // Merah jika SOS
                      shape: BoxShape.circle,
                    ),
                    child: Icon(
                      style['icon'],
                      color: isSOS ? Colors.white : style['color'],
                      size: 24,
                    ),
                  ),
              ...
              if (report.photoUrl.isNotEmpty &&
                  ...
                  !report.photoUrl.contains('placeholder')) ...[
                ClipRRect(
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  icon: const Icon(Remix.direction_fill, color: Colors.white),
                  label: const Text(
                    "Navigasi ke Lokasi",
                    style: TextStyle(
                      fontWeight: FontWeight.bold,
                  ...
                  style: ElevatedButton.styleFrom(
                    backgroundColor: isSOS ? Colors.red : Colors.blueAccent,
